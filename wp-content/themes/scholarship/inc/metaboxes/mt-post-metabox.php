...
add_action( 'add_meta_boxes', 'scholarship_post_meta_options' );

if ( ! function_exists( 'scholarship_post_meta_options' ) ) :

    function  scholarship_post_meta_options() {
        add_meta_box(
            'scholarship_post_meta',
            esc_html__( 'Post Options', 'scholarship' ),
            'scholarship_post_meta_callback',
            'post',
            'normal',
            'high'
        );
...
$scholarship_post_sidebar_options = array(
...
if ( ! function_exists( 'scholarship_post_meta_callback' ) ) :

    /**
     * Callback function for post option
     */
	function scholarship_post_meta_callback() {
		global $post, $scholarship_post_sidebar_options;
		...
		wp_nonce_field( basename( __FILE__ ), 'scholarship_post_meta_nonce' );
?>
			...
			<div class="mt-metabox-content-wrapper">
				...
				<div class="mt-single-meta" id="mt-metabox-sidebar">
					...
					<div class="mt-meta-options-wrap">
						<div class="buttonset">
							<?php
			                   	foreach ( $scholarship_post_sidebar_options as $field ) {
			                    	$scholarship_post_sidebar = get_post_meta( $post->ID, 'single_post_sidebar', true );
                                    $scholarship_post_sidebar = ( $scholarship_post_sidebar ) ? $scholarship_post_sidebar : 'default_sidebar';
			                ?>
			                    	<input type="radio" id="<?php echo esc_attr( $field['id'] ); ?>" value="<?php echo esc_attr( $field['value'] ); ?>" name="single_post_sidebar" <?php checked( $field['value'], $scholarship_post_sidebar ); ?> />
...
add_action( 'save_post', 'scholarship_save_post_meta' );

if ( ! function_exists( 'scholarship_save_post_meta' ) ) :
     ...
     * @since 1.0.0
     */
    function scholarship_save_post_meta( $post_id ) {

        global $post;

        // Verify the nonce before proceeding.
        $scholarship_post_nonce   = isset( $_POST['scholarship_post_meta_nonce'] ) ? $_POST['scholarship_post_meta_nonce'] : '';
        $scholarship_post_nonce_action = basename( __FILE__ );

        //* Check if nonce is set...
        if ( ! isset( $scholarship_post_nonce ) ) {
            return;
        }

        //* Check if nonce is valid...
        if ( ! wp_verify_nonce( $scholarship_post_nonce, $scholarship_post_nonce_action ) ) {
            return;
        }
        ...
        /*Post sidebar*/
        if ( isset( $_POST['single_post_sidebar'] ) ) {
            // We validate making sure that the option is something we can expect.
            $value = in_array( $_POST['single_post_sidebar'], array( 'no_sidebar', 'left_sidebar', 'right_sidebar', 'no_sidebar_center', 'default_sidebar' ) ) ? $_POST['single_post_sidebar'] : 'default_sidebar';
            // We update our post meta.
            update_post_meta( $post_id, 'single_post_sidebar', $value );
        }
