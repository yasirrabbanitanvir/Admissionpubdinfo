...
if ( ! class_exists( 'Scholarship_Settings' ) ) :

	class Scholarship_Settings {
		...
		public function __construct() {
			...
			add_action( 'wp_loaded', array( __CLASS__, 'scholarship_hide_notices' ) );
			add_action( 'wp_loaded', array( $this, 'scholarship_admin_notice' ) );
		...
		public function scholarship_admin_menu() {
			...
			add_theme_page( sprintf( esc_html__( '%1$s Settings', 'scholarship' ), $theme_name ), sprintf( esc_html__( '%1$s Settings', 'scholarship' ), $theme_name ) , 'edit_theme_options', 'scholarship-settings', array( $this, 'get_started_screen' ) );
		}
		...
		public function about_theme_styles( $hook ) {
			global $scholarship_theme_version;
			$theme_notice_option = get_option( 'scholarship_admin_notice_welcome' );
			if ( $theme_notice_option ) {
				wp_enqueue_style( 'mt-theme-review-notice', get_template_directory_uri() . '/inc/theme-settings/assets/css/theme-review-notice.css', array(), esc_attr( $scholarship_theme_version ) );
			}

			if ( 'appearance_page_scholarship-settings' != $hook && 'themes.php' != $hook ) {
				return;
			}

			wp_enqueue_style( 'mt-theme-settings-style', get_template_directory_uri() . '/inc/theme-settings/assets/css/settings.css', array(), $scholarship_theme_version );
		}
		...
		public function about_theme_scripts( $hook ) {
			...
			$theme_notice_option = get_option( 'scholarship_admin_notice_welcome' );
			if ( $theme_notice_option ) {
				wp_enqueue_script( 'mt-theme-review-notice', get_template_directory_uri() . '/inc/theme-settings/assets/js/theme-review-notice.js', array( 'jquery' ), esc_attr( $scholarship_theme_version ) );
				...
				wp_localize_script( 'mt-theme-review-notice', 'mtaboutObject', array(
					...
					'_wpnonce'	=> wp_create_nonce( 'scholarship_admin_plugin_install_nonce' ),
					'action'	=> esc_html( $action )
				));
			}

			if ( 'appearance_page_scholarship-settings' != $hook ) {
				return;
			}

			$activated_plugins = apply_filters( 'scholarship_active_plugins', get_option('active_plugins') );
			...
			wp_enqueue_script( 'mt-theme-settings-script', get_template_directory_uri() . '/inc/theme-settings/assets/js/settings.js', array( 'jquery' ), esc_attr( $scholarship_theme_version ) );
		 ...
		 * Add admin notice.
		 */
		public function scholarship_admin_notice() {
		 ...
		 */
		public static function scholarship_hide_notices() {
	...
	?>
			<div id="mt-theme-message" class="updated notice scholarship-message">
				<a class="scholarship-message-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'activated' ), add_query_arg( 'scholarship-hide-notice', 'welcome' ) ), 'scholarship_hide_notices_nonce', '_scholarship_notice_nonce' ) ); ?>">
				...
				<p>
					<?php printf( esc_html__( 'Welcome! Thank you for choosing %1$s ! To fully take advantage of the best our theme can offer please make sure you visit our %2$s theme settings page %3$s.', 'scholarship' ), '<strong>'. esc_html( $theme_name ).'</strong>', '<a href="' . esc_url( admin_url( 'themes.php?page=scholarship-settings' ) ) . '">', '</a>' ); ?>
				</p>
				<p>
					<?php printf( esc_html__( 'Clicking get started will process to installation of %1$s Mystery Themes Demo Importer %2$s Plugin in your dashboard. After success it will redirect to the theme settings page.', 'scholarship' ), '<strong>', '</strong>' ); ?>
				</p>
				<div class="submit">
					<button class="mt-get-started button button-primary button-hero" data-done="<?php esc_attr_e( 'Done!', 'scholarship' ); ?>" data-process="<?php esc_attr_e( 'Processing', 'scholarship' ); ?>" data-redirect="<?php echo esc_url( wp_nonce_url( add_query_arg( 'scholarship-hide-notice', 'welcome', admin_url( 'themes.php' ).'?page=scholarship-settings&tab=demos' ) , 'scholarship_hide_notices_nonce', '_scholarship_notice_nonce' ) ); ?>">
						<?php printf( esc_html__( 'Get started with %1$s', 'scholarship' ), esc_html( $theme_name ) ); ?>
					</button>
				</div>
	...
	?>
			<div class="scholarship-theme-info mt-theme-info mt-clearfix">
				<h1 class="mt-about-title"> <?php echo esc_html( $theme_name ); ?> </h1>
				<div class="author-credit">
					<span class="theme-version"><?php printf( esc_html__( 'Version: %1$s', 'scholarship' ), $scholarship_theme_version ); ?></span>
					<span class="author-link"><?php printf( wp_kses_post( 'By <a href="%1$s" target="_blank">%2$s</a>', 'scholarship' ), $author_uri, $author_name ); ?></span>
				</div>
			</div><!-- .scholarship-theme-info -->

			<div class="mt-upgrader-pro">
				<div class="mt-upgrade-title-wrap">
					<h3 class="mt-upgrader-title"><?php esc_html_e( 'Upgrade to Premium Version', 'scholarship' ); ?></h3>
					<div class="mt-upgrader-text"><?php esc_html_e( 'Upgrade to pro version for additional features and better supports.', 'scholarship' ); ?></div>
				</div>
				<div class="mt-upgrader-btn"> <a href="<?php echo esc_url( 'https://mysterythemes.com/wp-themes/scholarship-pro/' ); ?>" target="_blank" class="button button-primary"><?php esc_html_e( 'Unlock Features With Pro', 'scholarship' ); ?></a> </div>
			</div><!-- .mt-upgrader-pro -->

			<div class="mt-nav-tab-content-wrapper">
				<div class="nav-tab-wrapper">

					<a class="nav-tab <?php if ( empty( $_GET['tab'] ) && $_GET['page'] == 'scholarship-settings' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'scholarship-settings' ), 'themes.php' ) ) ); ?>">
						<span class="dashicons dashicons-admin-appearance"></span> <?php esc_html_e( 'Get Started', 'scholarship' ); ?>
					</a>

					<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'demos' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'scholarship-settings', 'tab' => 'demos' ), 'themes.php' ) ) ); ?>">
						<span class="dashicons dashicons-download"></span> <?php esc_html_e( 'Demos', 'scholarship' ); ?>
					</a>
					
					<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'free_vs_pro' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'scholarship-settings', 'tab' => 'free_vs_pro' ), 'themes.php' ) ) ); ?>">
						<span class="dashicons dashicons-dashboard"></span> <?php esc_html_e( 'Free Vs Pro', 'scholarship' ); ?>
					</a>

					<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'changelog' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'scholarship-settings', 'tab' => 'changelog' ), 'themes.php' ) ) ); ?>">
				...
				<?php $this->intro(); ?>
					<div class="mt-nav-content-wrap">
						<div class="theme-features-wrap welcome-panel">
							<h4><?php esc_html_e( 'Here are some useful links for you to get started', 'scholarship' ); ?></h4>
							<div class="under-the-hood two-col">
								<div class="col">
									<h3><?php esc_html_e( 'Next Steps', 'scholarship' ); ?></h3>
									<ul>
										<li>
											<a href="<?php echo esc_url( admin_url( 'customize.php' ).'?autofocus[section]=title_tagline' ); ?>" target="_blank" class="welcome-icon dashicons-visibility"><?php esc_html_e( 'Setup site logo', 'scholarship' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'customize.php' ).'?autofocus[panel]=scholarship_design_settings_panel' ); ?>" target="_blank" class="welcome-icon dashicons-text-page"><?php esc_html_e( 'Manage Post/Page/Archive layouts', 'scholarship' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'customize.php' ).'?autofocus[section]=social_icons_section' ); ?>" target="_blank" class="welcome-icon dashicons-networking"><?php esc_html_e( 'Manage Social Icons', 'scholarship' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'nav-menus.php' ) ); ?>" target="_blank" class="welcome-icon welcome-menus"><?php esc_html_e( 'Manage menus', 'scholarship' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'widgets.php' ) ); ?>" target="_blank" class="welcome-icon welcome-widgets"><?php esc_html_e( 'Manage widgets', 'scholarship' ); ?></a>
										</li>
									</ul>
								</div>

								<div class="col">
									<h3><?php esc_html_e( 'More Actions', 'scholarship' ); ?></h3>
									<ul>
										<li>
											<a href="<?php echo esc_url( $doc_url ); ?>" target="_blank" class="welcome-icon dashicons-media-text"><?php esc_html_e( 'Documentation', 'scholarship' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( $pro_theme_url ); ?>" target="_blank" class="welcome-icon dashicons-migrate"><?php esc_html_e( 'Premium version', 'scholarship' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( $support_url ); ?>" target="_blank" class="welcome-icon dashicons-businesswoman"><?php esc_html_e( 'Need theme support?', 'scholarship' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( $review_url ); ?>" target="_blank" class="welcome-icon dashicons-thumbs-up"><?php esc_html_e( 'Review theme', 'scholarship' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( 'https://wpallresources.com/' ); ?>" target="_blank" class="welcome-icon dashicons-admin-users"><?php esc_html_e( 'WP Tutorials', 'scholarship' ); ?></a>
										</li>
									</ul>
								</div>
							</div>
				...
				<?php $this->intro(); ?>
					<div class="mt-nav-content-wrap">
						<div class="mt-theme-demos rendered">
							...
							?>
									<span class="configure-msg"><?php esc_html_e( 'Demos file not configured, you may have not activated mysterythemes demo importer plugin yet!!', 'scholarship' ); ?></span>
							<?php
								} else {
							?>
						...
						<table>
							...
							<tbody>
								...
								<tr>
									...
									<a href="<?php echo esc_url( $pro_theme_url ); ?>" class="button button-primary" target="_blank"><?php esc_html_e( 'Buy Pro', 'scholarship' ); ?></a>
	...
	<?php
	    ...
	    public function scholarship_admin_footer_text( $text ) {
	        ...
	        if ( 'appearance_page_scholarship-settings' == $screen->id ) {

	        	$theme 		= wp_get_theme( get_template() );
				$theme_name = $theme->get( 'Name' );

	            $text = sprintf( __( 'If you like <strong>%1$s</strong> please leave us a %2$s rating. A huge thank you from <strong>Mystery Themes</strong> in advance &#128515!', 'scholarship' ), esc_html( $theme_name ), '<a href="https://wordpress.org/support/theme/scholarship/reviews/?filter=5#new-post" class="theme-rating" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>' );
		...
		?>
				<div id="mt-demo-import-plugin-popup">
					<div class="mt-popup-inner-wrap">
						<?php
							if ( is_plugin_active( 'mysterythemes-demo-importer/mysterythemes-demo-importer.php' ) ) {
								echo '<span class="mt-plugin-message">'.esc_html__( 'You can import available demos now!', 'scholarship' ).'</span>';
							} else {
								if ( ! file_exists( $demo_importer_plugin ) ) {
						?>
									<span class="mt-plugin-message"><?php esc_html_e( 'Mystery Themes Demo Importer Plugin is not installed!', 'scholarship' ); ?></span>
									<a href="javascript:void(0)" class="mt-install-demo-import-plugin" data-process="<?php esc_attr_e( 'Installing & Activating', 'scholarship' ); ?>" data-done="<?php esc_attr_e( 'Installed & Activated', 'scholarship' ); ?>">
										<?php esc_html_e( 'Install and Activate', 'scholarship' ); ?>
									</a>
						<?php
								} else {
						?>
									<span class="mt-plugin-message"><?php esc_html_e( 'Mystery Themes Demo Importer Plugin is installed but not activated!', 'scholarship' ); ?></span>
									<a href="javascript:void(0)" class="mt-activate-demo-import-plugin" data-process="<?php esc_attr_e( 'Activating', 'scholarship' ); ?>" data-done="<?php esc_attr_e( 'Activated', 'scholarship' ); ?>">
		...
		public function activate_demo_importer_plugin() {
			if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'scholarship_admin_plugin_install_nonce' ) ) {
...
return new Scholarship_Settings();